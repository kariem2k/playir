...
void CCCameraBase::setNearFar(const float zNear, const float zFar)
{
    this->zNear = zNear;
    this->zFar = zFar;
    setPerspective( perspective * orientedAspectRatio );
}
...
void CCCameraBase::setCollideables(const CCList<CCCollideable> *inCollideableList)
{
...
void CCCameraBase::ExtractFrustum()
{
	...
	const float *modv = getViewMatrix().data();
	float clip[16];
	float t;
...
void GLResetMatrix()
{
	...
	{
		ASSERT( CCCameraBase::CurrentCamera->currentPush == 0 );
