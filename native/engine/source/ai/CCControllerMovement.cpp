 ...
 *-----------------------------------------------------------
 * File Name   : CCControllerMovement.cpp
 *-----------------------------------------------------------
 */
...
CCControllerMovement::CCControllerMovement(CCCollideable *inObject)
{
    object = inObject;
...
bool CCControllerMovement::update(const float delta)
{
    if( findingTarget )
    {
        // Target
        {
            const CCVector3 &position = object->getConstPosition();
...
void CCControllerMovement::setTarget(const CCVector3 &target)
{
    ...
    if( positionTarget != object->getConstPosition() )
    {
        findingTarget = true;
    }
}
...
{
    CCVector3 &position = object->getPosition();
