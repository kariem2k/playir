...
{
	CCLAMBDA_4( Create, CCTextureBase, that, bool, loaded, bool, generateMipMap, CCLambdaSafeCallback*, callback, {
    ...
    CCLAMBDA_FINISH_6( Load, CCTextureBase, that, CCText, path, CCResourceType, resourceType, bool, generateMipMap, CCLambdaSafeCallback*, callback, bool, loaded,
    ...
    // Finish on the jobs thread
    {
    	if( gEngine != NULL )
    	{
    		gEngine->jobsToEngineThread( new Create( that, loaded, generateMipMap, callback ) );
    	}
    });

    gEngine->engineToJobsThread( new Load( this, path, resourceType, generateMipMap, callback, false ) );
}
