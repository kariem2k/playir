...
CCPrimitiveSquare::CCPrimitiveSquare(const char *primitiveID) :
...
void CCPrimitiveSquare::destruct()
{
    ...
    {
        ASSERT( customVertexPositionBuffer == vertices );
...
void CCPrimitiveSquare::setupForwardFacing(const float width, const float height, const float zPosition)
{
	CCVector3FillPtr( &scale, width, height, 1.0f );
}
