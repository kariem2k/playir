...
void CCTile3DButton::setupTextured(const char *textureName, CCResourceType resourceType, CCLambdaCallback *onDownloadCallback, const bool mipmap)
{
    setTileSize();
    ...
    setTileTexture( textureName, resourceType, new DownloadedCallback( this, onDownloadCallback ), mipmap );
}
...
{
    setTileSize();
    setTileTexture( textureName, Resource_Packaged, NULL, mipmap );
...
void CCTile3DButton::setupTexturedFit(const float width, const float height, const char *textureName, const bool mipmap)
{
    ...
    setTileTexture( textureName, Resource_Unknown, new DownloadedCallback( this, width, height ), mipmap, false );
}
...
void CCTile3DButton::setTileSize(const float width, const float height)
{
    ...
    tileSquare->setupForwardFacing( collisionSize.width, collisionSize.height );
}
                                    ...
                                    const bool mipmap, const bool load)
{
    if( tileSquare != NULL )
    {
        tileSquare->setTexture( textureName, resourceType, onDownloadCallback, mipmap, load );
    }
}
...
{
    ASSERT( tileSquare != NULL && tileSquare->getTextureHandleIndex() != -1 );
    ...
    gEngine->textureManager->getTextureAsync( tileSquare->getTextureHandleIndex(), new OnLoadCallback( this ) );
}
...
void CCTile3DButton::setTileTexturedWidth(const float width)
{
    ...
    gEngine->textureManager->getTextureAsync( tileSquare->getTextureHandleIndex(), new OnLoadCallback( this, width ) );
}
...
void CCTile3DButton::setTileTexturedHeight(const float height)
{
    ...
    gEngine->textureManager->getTextureAsync( tileSquare->getTextureHandleIndex(), new OnLoadCallback( this, height ) );
}
...
void CCTile3DButton::setTileTexturedFit(const float width, const float height, const bool crop)
{
    ...
    gEngine->textureManager->getTextureAsync( tileSquare->getTextureHandleIndex(), new OnLoadCallback( this, width, height, crop ) );
}
