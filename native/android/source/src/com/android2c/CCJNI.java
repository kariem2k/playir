...
public class CCJNI
{
    ...
    public static void SetActivity(CCActivity inActivity)
    {
        ...
        if( Environment.MEDIA_MOUNTED.equals( state ) )
        {
			...
			final String appsPath = externalStorage.getAbsolutePath() + "/Apps";
			final String namePath = appsPath + "/" + Activity.getString( CCActivity.AppName() );
			final String packagePath = namePath + "/" + PackageName;
			{
				final File packageDirectory = new File( packagePath );
				if( packageDirectory.exists() == false )
				{
					final File nameDirectory = new File( namePath );
					if( nameDirectory.exists() == false )
					{
						final File appsDirectory = new File( appsPath );
						if( appsDirectory.exists() == false )
						{
							appsDirectory.mkdir();
						}
						nameDirectory.mkdir();
	...
	private static void URLManagerProcessRequest(final String url)
	{
 	...
 	private static void WebJSClose()
	{
  	...
  	private static void VideoViewStop()
 	{
  	...
  	// Google Services
  	private static void GoogleServicesRegister()
  	{
  		Activity.runOnUiThread(new Runnable()
		{
		    public void run()
		    {
		    	Activity.googleServicesRegister();
