...
void CCDeviceURLManager::processRequest(CCURLRequest *inRequest)
{
	...
	static jmethodID mid = jniEnv->GetStaticMethodID( jniClass, "URLManagerProcessRequest", "(Ljava/lang/String;[BILjava/lang/String;)V" );
	CCASSERT( mid != 0 );

	// Call the function
	jstring jURL = jniEnv->NewStringUTF( inRequest->url.buffer );


	jbyteArray jPostData = NULL;
	jstring jPostBoundary = NULL;

	if( inRequest->postBody.length > 0 )
	{
		jPostData = jniEnv->NewByteArray( inRequest->postBody.length );
		if( jPostData != NULL )
		{
			jboolean isCopy;
			void *data = jniEnv->GetPrimitiveArrayCritical( (jarray)jPostData, &isCopy );
			memcpy( data, inRequest->postBody.buffer, inRequest->postBody.length );
		}
		jPostBoundary = jniEnv->NewStringUTF( CCPOST_BOUNDARY );
	}

	jniEnv->CallStaticVoidMethod( jniClass, mid, jURL, jPostData, inRequest->postBody.length, jPostBoundary );

	if( jPostData != NULL )
	{
		jniEnv->DeleteLocalRef( jPostData );
	}
}
