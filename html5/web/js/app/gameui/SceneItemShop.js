...
function SceneItemShop(title, itemCode, consumable, inAppPurhcaseValue, facebookValue, twitterValue, callback)
{
    ...
    // If we're making this purchase from another app, fallback to using it's default purchase ID
    if( window.LAUNCH_APP_ID !== window.APP_ID )
    {
        itemCode = window.DEFAULT_PURCHASE_ID;
    }

    if( window.tizen )
    {
        inAppPurhcaseValue = false;
        twitterValue = false;
    }
...
SceneItemShop.prototype.setup = function()
{
    ...
    if( !!SceneItemShop.facebookValue )
    {
        ...
        tile.setupTexturedWidth( 25.0, "icon_facebook.png", function()
        {
            self.requestResize();
        });
        ...
        // Text
        {
            ...
            tile.setupText( "Mention us +" + SceneItemShop.facebookValue, camera.targetHeight * 0.05 );
            tile.setTextColour( new CCColour() );
    ...
    if( !!SceneItemShop.twitterValue )
    {
        ...
        tile.setupTexturedWidth( 25.0, "icon_twitter.png", function()
        {
            self.requestResize();
        });
    ...
    // In-App Purchase
    if( !!SceneItemShop.inAppPurhcaseValue )
    {
        if( ( CCEngine.DeviceType === "iOS" || CCEngine.DeviceType === "Android" || CCEngine.DeviceType === "WindowsPhone" ) &&
            !window.amazon && !window.samsung )
        {
            tile = new CCTile3DButton( this );
            this.tileInAppPurchase = tile;

            var iconURL = "icon_store";
...
SceneItemShop.prototype.renderVisibleObject = function(object, inCamera, pass, alpha)
{
...
SceneItemShop.prototype.inAppPurchase = function()
{
    ...
    {
        AlertsManager.TimeoutAlert( "unable to connect\ncheck your internet connection\nand try again", 5.0 );
        return;
    }
