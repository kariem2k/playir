...
AlertsManager.Hide = function(message)
{
    var AlertsList = AlertsManager.AlertsList;
...
AlertsManager.Alert = function(message, callback)
{
    var AlertsList = AlertsManager.AlertsList;
...
AlertsManager.Notification = function(title, message, callback, timeout, colour, scale)
{
    ...
    for( i=0; i<AlertsList.length; ++i )
    {
        ...
        {
            var y = alert.tileAlert.position[1] - alert.tileAlert.collisionBounds[1];
            if( y < yOffset )
            {
                yOffset = y;
            }
    ...
    {
        alert = new SceneUINotification( title, message );
        if( callback )
        {
            alert.closeCallback = callback;
        }
        ...
        if( yOffset !== CC_MAXFLOAT )
        {
            alert.setYOffset( yOffset );
        }
        if( scale )
        {
            alert.scale = scale;
...
AlertsManager.UserMessage = function(userID, message, callback, timeout, colour)
{
    ...
    {
        alert = new SceneUINotification( userID, message, userID );
        if( callback )
        {
            alert.closeCallback = callback;
        }
