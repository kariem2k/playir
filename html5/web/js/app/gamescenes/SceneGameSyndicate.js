...
SceneGameSyndicate.prototype.construct = function(mapID, mapType)
{
    ...
    this.keyPressedTimer = 0.0;
    this.keySyncTimer = 0.0;
};
SceneGameSyndicate.KeySyncTime = 0.25;
...
SceneGameSyndicate.prototype.setup = function()
{
    ...
    this.setupPlayerDestinationIndicator( "fx_character_highlight_texture.png",
...
SceneGameSyndicate.prototype.updateScene = function(delta)
{
    ...
    if( this.keyPressed )
    {
        this.keyPressedTimer += delta;
        this.keySyncTimer += delta;
    }
...
SceneGameSyndicate.prototype.playerShootAtTarget = function(target, sync)
{
    this.playerCharacter.controller.shoot( target );
    if( sync )
    {
        if( this.isOnlineGame() )
        {
            MultiplayerManager.SyncShootAtTarget( this, target );
        }
    }
};
...
SceneGameSyndicate.prototype.onKeyboard = function(event, key, pressed)
{
    if( event.metaKey )
    {
        return false;
    ...
    else
    {
        ...
        if( key === "up" || key === "down" || key === " " )
        {
            var direction = [0.0,0.0,-1.0];
            if( key === "up" || key === " " )
            {
                CC.Vector3RotateY( direction, this.camera.rotation[1] );
            }
            else if( key === "down" )
            {
                CC.Vector3RotateY( direction, this.camera.rotation[1]+180.0 );
            }
            vec3.scale( direction, direction, this.keyPressedTimer > 0.25 ? 30.0 : 15.0 );

            var player = this.playerCharacter;
            vec3.add( direction, player.position, direction );

            //var positionIndicatorVector = direction;
            //this.playerDestinationIndicator.setPositionXZ( positionIndicatorVector[0], positionIndicatorVector[2] );
            //this.playerDestinationIndicator.setColourAlpha( 1.0 );

            if( key === " " )
            {
                this.playerShootAtTarget( direction, true );
            }
            else
            {
                if( this.keySyncTimer >= SceneGameSyndicate.KeySyncTime )
                {
                    this.keySyncTimer -= SceneGameSyndicate.KeySyncTime;
                    this.onGotoLocation( direction, true );
                }
                else
                {
                    this.onGotoLocation( direction, false );
                }
            }
        }
        else if( key === "left" || key === "right" )
        {
            ...
            {
                this.cameraRotationTarget = vec3.clone( this.camera.rotation );
            }
            var rotationAmount = this.keyPressedTimer > 0.25 ? 14.0 : 7.0;
            if( key === "left" )
            {
                this.cameraRotationTarget[1] += rotationAmount;
            }
            else if( key === "right" )
            {
                this.cameraRotationTarget[1] -= rotationAmount;
            }
            CC.ClampRotation( this.cameraRotationTarget[1] );
        }
    }
};
