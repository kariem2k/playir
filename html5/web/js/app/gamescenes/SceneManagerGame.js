...
SceneManagerGame.DefaultPlayerTypes = [
    {
        type:"iBot",
        icon:"resources/androids/iBot_icon.png",
        obj:"resources/androids/bots/iBot_body.obj",
        tex:"resources/androids/bots/iBot_diffuse.jpg"
    },
    {
        type:"winBot",
        icon:"resources/androids/androBot_icon.png",
        obj:"resources/androids/bots/winBot_body.obj",
        tex:"resources/androids/bots/winBot_diffuse.jpg"
    }];
SceneManagerGame.HelpImage = "resources/androids/phonewars_helpscreen.jpg";
...
SceneManagerGame.prototype.construct = function()
{
    ...
    if( gameInfoString )
    {
        var gameInfos = JSON.parse( gameInfoString );
        if( gameInfos )
        {
            for( var i=0; i<gameInfos.length; ++i )
            {
                var gameInfo = gameInfos[i];
                if( gameInfo.id === CLIENT_ID )
                {
...
SceneManagerGame.prototype.setup = function()
{
    ...
    {
        tile = new CCTile3DButton( this );
        tile.setupTexturedWidth( camera.targetWidth * 0.085, "resources/common/uimenu/ui_back.png" );
...
SceneManagerGame.prototype.handleBackButton = function()
{
    ...
    {
        this.scenePlayScreen.launchMulti();
        return true;
    }
    else if( this.gameState === SceneManagerGame.GameState_CharacterSelectScreen ||
...
SceneManagerGame.prototype.updateGameInfo = function(gameInfo)
{
    var facebookID = gameInfo.facebookID;
    if( facebookID )
    {
        CCAPIFacebook.APP_ID = facebookID;
    }

    var backgroundImage = gameInfo.backgroundImage;
    if( backgroundImage )
    {
    ...
    if( gameInfo.helpImage )
    {
        SceneManagerGame.HelpImage = gameInfo.helpImage;
        gEngine.textureManager.getTextureHandle( SceneManagerGame.HelpImage );
    }

    if( gameInfo.defaultMapImage )
    {
        SceneGameMap.DefaultMapImage = gameInfo.defaultMapImage;
        gEngine.textureManager.getTextureHandle( SceneGameMap.DefaultMapImage );
    }

    var i;
    var playerTypes = gameInfo.playerTypes;
...
SceneManagerGame.prototype.disconnected = function()
{
...
SceneManagerGame.prototype.syncUpdate = function(jsonData)
{
    ...
    {
        this.updateGameInfo( jsonData.gameInfo );
    }
        ...
        else if( result )
        {
            if( jsonData.disconnected )
            {
                ...
                if( map )
                {
                    ...
                    {
                        var aiUpdater = new CharacterControllerAI( map.getPathFinderNetwork() );
                        disconnectedPlayer.setupAI( aiUpdater );

                        if( !map.gameStarted() )
                        {
                            aiUpdater.enable( false );
                        }

                        aiUpdater.scanWaypoints( CC.SQUARE( map.getMapBounds().width ) );
                        aiUpdater.setWaypointCycle( CharacterControllerAI.cycle_aiNodes );
                        aiUpdater.setMovementMagnitude( 0.75 );
                        aiUpdater.setEnemy( player );
...
SceneManagerGame.prototype.spawnOfflineGame = function()
{
    ...
    // Spawn walls
    {
        ...
        {
            map.spawnStaticObject( "sandbag", objFile, texFile, null, sandbagWidth, 90.0, sandbagLocations[i] );
        }
    }
...

    {
        ...
        aiUpdater.enable( false );
        aiUpdater.scanWaypoints( CC.SQUARE( 75.0 ) );
        aiUpdater.setWaypointCycle( CharacterControllerAI.cycle_aiNodes );
        aiUpdater.setMovementMagnitude( 0.5 );
...
SceneManagerGame.prototype.spawnOfflinePlayers = function()
{
    ...
    // Spawn player2
    var player2;
    if( String.Contains( this.playerType, this.playerTypes[0].type ) )
    {
...
SceneManagerGame.prototype.isUsablePlayer = function()
{
    var playerTypeInfo = this.getPlayerTypeInfo( this.playerType );
    if( playerTypeInfo && playerTypeInfo.premiumCode )
    {
        var playerItemCode = playerTypeInfo.premiumCode;
        if( !SceneItemShop.HasPurchasedNonConsumable( playerItemCode ) )
        {
            new SceneItemShop( "Get 1,000 coins to unlock\nall game characters", "topup_1", true, "unlock", "unlock", "unlock", function (type, itemCode, value)
            {
...
SceneManagerGame.prototype.createFlag = function()
{
    ...
    {
        var file = "resources/common/flags/flag_";
