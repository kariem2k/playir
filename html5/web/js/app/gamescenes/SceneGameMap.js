...
SceneGameMap.DefaultMapImage = "level_map_diffuse.jpg";
...
SceneGameMap.prototype.construct = function(mapID, mapType)
{
    ...
    this.cameraRotationTargetSpeed = 180.0;
...

    {
        ...
        this.ground.setTransparent();
        this.ground.setColourAlpha( 0.0, false );
...
SceneGameMap.prototype.setMapTexture = function(filename)
{
    if( this.ground )
    {
        var self = this;
        this.ground.setTransparent();
        this.ground.setColourAlpha( 0.0, true );
        this.ground.primitive.setTexture( filename, undefined, undefined, undefined, function()
        {
            self.ground.setColourAlpha( 1.0, true, function()
            {
                self.ground.setTransparent( false );
            });
        });
    }
};
...
SceneGameMap.prototype.setMapTextureIndex = function(index)
{
    if( this.ground )
    {
        this.ground.primitive.setTextureIndex( index );

        this.setTransparent();
        var self = this;
        this.ground.setColourAlpha( 1.0, true, function()
        {
            self.ground.setTransparent( false );
        });
    }
...
{
    {
        ...
        for( var i=0; i<particles.length; ++i )
        {
            ...
            particle.rotateY( delta * positive );
        }
    }
    ...
    if( this.cameraRotationTarget )
    {
        ...
        camera.setRotationX( CC.ToRotation( camera.rotation[0], this.cameraRotationTarget[0], delta * this.cameraRotationTargetSpeed ) );
        camera.setRotationY( CC.ToRotation( camera.rotation[1], this.cameraRotationTarget[1], delta * this.cameraRotationTargetSpeed ) );
        camera.setRotationZ( CC.ToRotation( camera.rotation[2], this.cameraRotationTarget[2], delta * this.cameraRotationTargetSpeed ) );
...
SceneGameMap.prototype.createParticles = function()
{
    var self = this;

    var commonLevelsPath = "level_";
...

    {
        ...
        object.setColour( gColour.set( 1.0, 0.5 ) );
...
SceneGameMap.prototype.spawnStaticObject = function(objectID, obj, tex, options, width, rotation, location)
{
    ...
    {
        if( objectID === this.staticObjects[i].objectID )
        {
            object = this.staticObjects[i];
    ...
    if( !object )
    {
        object = new CCCollideableStaticMesh( objectID );
        object.setScene( this );
        object.setTransparent();
...
{
    character.setupShootingIndicator( "fx_character_shooting_texture.png",
                                      gColour.setRGBA( 1.0, 0.5, 0.0, 0.9 ) );

    character.setupCharacterIndicator( "fx_character_highlight_texture.png",
...
SceneGameMap.prototype.syncUpdate = function(jsonData)
{
    ...
    else if( jsonData.SyncShootAtTarget )
    {
        playerID = jsonData.SyncShootAtTarget;
        this.SyncShootAtTarget( playerID, jsonData.targetLocation );
    }
...
SceneGameMap.prototype.SyncShootAtTarget = function(playerID, targetLocation)
{
    var player = this.getPlayerFromID( playerID );
    if( player )
    {
        player.controller.shoot( targetLocation );
    }
};
