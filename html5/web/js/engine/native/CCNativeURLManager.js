...
CCURLManager.prototype.requestURL = function(url, postData, callback, priority, cacheFile, cacheFileTimeoutSeconds, timeout, returnBinary)
{
    ...
    var nativeCommand = 'CCURLManager.requestURL;' + request.id + ';' + url + ';';

    if( postData && typeof( postData ) === "object" )
    {
        if( !CCURLManager.TempPostDataIndex )
        {
            CCURLManager.TempPostDataIndex = 1;
        }
        var tmpFile = "_postdata" + CCURLManager.TempPostDataIndex++ + ".tmp";
        CCFileManager.Save( tmpFile, JSON.stringify( postData ) );
        nativeCommand += tmpFile;
    }
    nativeCommand += ';' + priority + ';';

    if( cacheFile )
    {
        nativeCommand += cacheFile;
    }
    nativeCommand += ';';

    nativeCommand += cacheFileTimeoutSeconds + ';' + timeout + ';';
    ...
    CCEngine.NativeUpdateCommands += nativeCommand;
};
