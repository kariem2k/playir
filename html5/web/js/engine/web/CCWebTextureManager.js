...
CCTextureManager.prototype.assignTextureIndex = function(url)
{
    ...
    var OnDowloadFunction = function(textureHandle)
    {
        return function()
        {
            self.downloadedImage( textureHandle );
        };
    };

    var OnErrorFunction = function(textureHandle)
    {
        return function()
        {
            self.downloadError( textureHandle );
        };
    };
    ...
    textureHandle.image.onerror = new OnErrorFunction( textureHandle );
...
CCTextureManager.prototype.downloadedImage = function(textureHandle)
{
    //console.log( "textureHandle.image.onload", url );
    textureHandle.downloaded = true;
    if( textureHandle.onDownload )
    {
        gRenderer.pendingRender = true;
        for( var i=0; i<textureHandle.onDownload.length; ++i )
        {
...
CCTextureManager.prototype.downloadError = function(textureHandle)
{
    //console.log( textureHandle.filename, textureHandle.downloadRetries );
    if( !textureHandle.downloaded )
    {
        ...
        if( textureHandle.downloadRetries < 3 )
        {
            ...
            textureHandle.image.src = "";
            textureHandle.image.src = textureHandle.src;
            return;
        }

        console.log( "textureHandle.image.onerror", textureHandle.src );
        if( textureHandle.onDownload )
        {
