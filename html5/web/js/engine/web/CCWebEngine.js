...
CCEngine.prototype.updateWeb = function()
{
    ...
    if( window.DEBUG_IsLocalClient )
    {
		...
		if( this.timerFPS > 5.0 )
		{
			this.timerFPS -= 5.0;
			console.log( "FPS", 1.0/delta );
		}
    }
};
	...
	if( renderer.beginRender() )
	{
		...
		for( var cameraIndex=0; cameraIndex<camerasLength; ++cameraIndex )
		{
			...
			for( var pass=render_background; pass<render_finished; ++pass )
			{
				...
				CC.RenderVisibleObjects( camera, pass, false );
				...
				CC.RenderVisibleObjects( camera, pass, true );
...
CCEngine.WebViewOpen = function(url, onOpenCallback, onUpdateCallback, params)
{
	...
	var webViewOpenedTimer = 0;
	var webViewOpenedTimeout = 100;
	var WaitForWebViewToOpen = function()
	{
		webViewOpenedTimer += webViewOpenedTimeout;
		setTimeout( function()
		{
			if( !webView || webView.innerHeight === 0 )
			{
				if( webViewOpenedTimer < 5000 )
				{
...
CC.DeleteLocalStorageCache = function(key, data)
{
    if( window.localStorage )
    {
        CC.ASSERT( key );
