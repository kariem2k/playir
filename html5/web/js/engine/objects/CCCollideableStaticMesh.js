...
CCCollideableStaticMesh.prototype.loadModel = function(obj, tex, callback)
{
    ...
    CCModel3D.CacheModel( obj, true, function(model3d)
    {
        if( model3d )
        {
            ...
            while( self.model.models.length>0 )
            {
                var model = self.model.models[0];
                self.model.removeModel( model );
                model.destruct();
            }
            ...
            //self.getMovementInterpolator().setMovementY( self.collisionBounds[1]*0.5 );
            //self.collideableType = CC.collision_none;
