...
CCModel3D.prototype.removeTexture = function()
{
    this.primitive.removeTexture();
};
...
CCModel3D.CacheModel = function(url, moveVerticesToOrigin, callback, priority)
{
    ...
    if( !cachedModel )
    {
        ...
        {
            console.log( "CCModel3D.CacheModel queue", filename, priority );
        }
    ...
    {
        CCModel3D.CacheModelResultProcess( cachedModel, [callback], false );
    }
};
    ...
    if( !CCModel3D.CurrentLoad && CCModel3D.LoadingQueue.length > 0 )
    {
        ...
        primitive.load( filename, url, priority, function(succeeded)
        {
            if( succeeded )
            {
                ...
                {
                    primitive.moveVerticesToOriginAsync( function()
                    {
                        CCModel3D.CurrentLoad = null;
...
{
    if( !CCModel3D.ResultQueue )
    {
        CCModel3D.ResultQueue = [];
    }

    var result = {};
    result.cachedModel = cachedModel;
    result.callbacks = callbacks;
    CCModel3D.ResultQueue.push( result );

    if( CCModel3D.ResultQueue.length === 1 )
    {
        CCModel3D.CacheModelResultQueue();
    }
...
CCModel3D.CacheModelResultQueue = function()
{
    //setTimeout( function() {
        CCEngine.RunNextUpdate( function() {
            if( CCModel3D.ResultQueue.length > 0 )
            {
                var result = CCModel3D.ResultQueue.safePop();
                CCModel3D.CacheModelResultProcess( result.cachedModel, result.callbacks );
...
CCModel3D.CacheModelResultProcess = function(cachedModel, callbacks)
{
    if( cachedModel )
    {
        cachedModel.lastCacheTime = gEngine.lifetime;
    }

    var i;

    // Limit number of models to cache
    var ModelCache = CCModel3D.ModelCache;
    var length = ModelCache.length;
    if( length > 100 )
    {
        var oldestCacheTime = CC_MAXFLOAT;
        var oldestModel = null;
        for( i=0; i<length; ++i )
        {
            var cache = ModelCache[i];
            if( cache.lastCacheTime < oldestCacheTime )
            {
                oldestModel = cache;
            }
        }
        ...
        if( oldestModel )
        {
            ModelCache.remove( oldestModel );
            oldestModel.primitive.destruct();
        }
    }

    // Copy primitive from cache
