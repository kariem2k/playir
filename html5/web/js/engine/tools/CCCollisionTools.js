...
function CCCollisionManager(octreeSize)
{
	this.octree = new CCOctree( null, vec3.create(), octreeSize );
	this.pruneOctreeTimer = 0.0;
...
CCCollisionManager.prototype.addCollideable = function(collideable)
{
	this.collideables.add( collideable );
	CCOctree.AddObject( this.octree, collideable );
};


CCCollisionManager.prototype.removeCollideable = function(collideable)
{
    this.collideables.remove( collideable );
	CCOctree.RemoveObject( collideable );
};
                                      ...
                                      excludeInvisibles)
{
    ...
    for( var i=0; i<length; ++i )
	{
		...
		if( ignoreObjects && ignoreObjects.find( checkingObject ) !== -1 )
		{
			continue;
		}
