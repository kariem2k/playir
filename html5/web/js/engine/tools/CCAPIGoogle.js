...
{
	var url = "http://playir.com/google+/users/";
	url += photoID;
	url += ".jpg";
...
CCAPIGoogle.StartLogin = function(callback)
{
	...
	{
		var url = "http://playir.com/apis/google+/?auto";

		if( url.getDomain() !== SERVER_ROOT.getDomain() )
		{
			url += "&redirect_uri=" + SERVER_ROOT + "apis/google+/";
		}
...
CCAPIGoogle.ProcessLogin = function(webViewController, url, open)
{
	if( open )
	{
		...
		if( splitURL.length > 1 )
		{
			...
			var id;
			var name;
			for( i=0; i<parametersSplit.length; ++i )
			{
