 ...
 *-----------------------------------------------------------
 * File Name   : CCJNIGLView.java
 * Description : Creates the OpenGL view.
 * 				 Originally based off hello-gl2 sample provided in AndroidNDKr5b
...
class CCJNIGLView extends GLSurfaceView
{
	public static GL11 GLContext;
    static boolean FirstRun;			// Always reset our engine on creation of this class
    ...
    public synchronized static void runOnGLThread(Runnable runnable)
    {
    	pendingRunnables.add( runnable );
    }
    
    private synchronized static void runRunnables()
    {
    	if( pendingRunnables.size() > 0 )
    	{
        	for( int i=0; i<pendingRunnables.size(); ++i )
        	{
        		Runnable runnable = pendingRunnables.get( i );
    ...
    public CCJNIGLView(Context context)
    {
        super( context );
    ...
    private static class Renderer implements GLSurfaceView.Renderer
    {
        ...
        {
        	GLContext = (GL11)gl;
    		CCJNI.OnSurfaceChanged( FirstRun, width, height );
    ...
    static void CheckGLError(String prompt, EGL10 egl)
    {
        int error;
    ...
    public boolean onTouchEvent(final MotionEvent event)
    {
		...
		// Untouch everything on a cancel
    		...
    		for( int i=0; i<2; ++i )
    		{
	    		if( touches[i] )
	    		{
	    			...
	    			try
	    			{
	    		    	...
	    		    	xs[i] = x;
	    		    	ys[i] = y;
	    	    		CCJNI.ControlsHandleTouch( x, y, MotionEvent.ACTION_CANCEL, i );
	    			}
	    			catch( Exception e )
	    			{
	    	    		CCJNI.ControlsHandleTouch( xs[i], ys[i], MotionEvent.ACTION_CANCEL, i );
	    			}
					return true;
	    		}
    		}
    	...
    	// Cancel second touch, if we only have one
		if( pointerCount == 1 )
		{
			final int i = 1;
			if( touches[i] )
    	...
    	if( touchId < 0 || touchId >= 2 )
    	{
    		if( CCJNI.DEBUG )
        	{
    			Log.d( "TouchEvent", "Id out of range " + touchId );
        	}
    	}
    	else
    	{
