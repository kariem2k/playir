...
void CCDeviceURLManager::processRequest(CCURLRequest *inRequest)
{
	...
	{
		static int UniquePostFileIndex = 0;
		CCText uniquePostFile = "_postData";
		uniquePostFile += UniquePostFileIndex++;
		uniquePostFile += ".tmp";
		CCFileManager::SaveCachedFile( uniquePostFile.buffer, inRequest->postBody.buffer, inRequest->postBody.length );

		CCText compactedParamters = inRequest->url.buffer;
		compactedParamters += ",";
		compactedParamters += uniquePostFile;
		compactedParamters += ",";
		compactedParamters += CCPOST_BOUNDARY;
		csActionStack.add( new CSAction( "CCDeviceURLManager::processPostRequest, ", compactedParamters.buffer ) );
	}
	else
	{
